...
public class MainApp {
	...
	public void start(Stage primaryStage) throws Exception{
		...
		File file = new File("keystore path");
		...
		Label descrip = new Label("Set Username with atleast 3-12 characters");
		TextArea loginT = new TextArea();
		Label descripP = new Label("Set password with 6-12 characters");
		TextArea loginP = new TextArea();
		Button login = new Button("Click To Login");
		Button create = new Button("Click To Create Account");
		create.setVisible(false);
		create.setManaged(false);
		Button switcher = new Button("Click To Switch To Create Account");
		VBox loginHold = new VBox(5);
		loginHold.getChildren().addAll(descrip,loginT,descripP, loginP,login,create, switcher);
		...
		switcher.setOnAction(p->{
			if(create.isVisible() == false) {
				login.setVisible(false);
				login.setManaged(false);
				create.setVisible(true);
				create.setManaged(true);
				switcher.setText("Click To Switch To Login");
			}else {
				create.setVisible(false);
				create.setManaged(false);
				login.setVisible(true);
				login.setManaged(true);
				switcher.setText("Click To Switch To Create Account");
			}
		});
		create.setOnAction(j->{
			login.setDisable(true);
			if(userN == null && loginT.getText().length() > 3 && loginT.getText().length() < 13) {
				if(loginP.getText().length()>5 || loginP.getText().length()<13) {
			userN = loginT.getText();
			HashMap<String, String> pObj = new HashMap<String, String>();
			pObj.put("user", loginT.getText());
			pObj.put("password", loginP.getText());
			mSocket.emit("create", pObj);
				}
			}
			loginT.setText("");
			loginP.setText("");
		...
		login.setOnAction(e->{	
			login.setDisable(true);
			if(loginT.getText().length() > 3 && loginT.getText().length() < 13) {
			if(loginP.getText().length()>5 || loginP.getText().length()<13) {
			userN = loginT.getText();
			HashMap<String, String> pObj = new HashMap<String, String>();
			pObj.put("user", loginT.getText());
			pObj.put("password", loginP.getText());
			mSocket.emit("login", pObj);
				}
			}
			loginT.setText("");
			loginP.setText("");
		...
		mSocket.on("logged", new Emitter.Listener() {  
				@Override
			    public void call(Object... args) { 
						Platform.runLater(new Runnable(){
							@Override
